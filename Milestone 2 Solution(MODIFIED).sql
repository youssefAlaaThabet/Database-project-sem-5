...
insert into InstructorTeachCourse(cid,insid) values(SCOPE_IDENTITY(),@instructorId)
...
AS
SELECT Course.id,Course.name FROM Course LEFT OUTER JOIN StudentTakeCourse ON Course.id = StudentTakeCourse.cid --changed here
...
AS
BEGIN
if (exists(select * from CoursePrerequisiteCourse
e where cid=@cid))
begin
...
insert into Users values ('mohamed','fermo','thabet',0,'bon@','affifi')
